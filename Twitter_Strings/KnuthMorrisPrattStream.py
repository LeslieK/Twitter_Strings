...
class DFA(object):
    def __init__(self, pat):
        ...
        # X refers to state after matching pat on text[1:]
        # (shifted to the right 1 character)
        X = 0
        for j in range(1, self.M):
            for c in range(StringUtils.R):
                # on a mismatch: set the state transitions from j
                # to be identical to those from X
